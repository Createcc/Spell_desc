...
def get_spell_description(spell_id, spells_df, spelleffect_df, spellmisc_df, spellduration_df, spellradius_df, spellname_df, spellauraoptions_df):
    ...
    return process_description(description, spells_df, spelleffect_df, spellmisc_df, spellduration_df, spellradius_df, spellname_df, spellauraoptions_df, spell_id)

def process_description(description, spells_df, spelleffect_df, spellmisc_df, spellduration_df, spellradius_df, spellname_df, spellauraoptions_df, parent_spell_id):
    pattern = re.compile(
    ...
    while '$' in description:
        ...
        for match in matches:
            (at_spelldesc, spell_desc_id, at_spellname, spellname_id, q_s, q_s_id, q_a, q_a_id, 
             s_full, s_index, digits_full, spell_id, spell_index, 
             d_full, d_index, spell_d_id, spell_d_index, 
             t_full, t_index, spell_t_id, spell_t_index, 
             a_full, a_index, spell_a_id, spell_a_index, 
             D_full, D_index, spell_D_id, spell_D_index, 
             a_lower_full, a_lower_index, spell_a_lower_id, spell_a_lower_index, 
             T_full, T_index, spell_T_id, spell_T_index,
             u_full, u_index, spell_u_id, spell_u_index,
             m_full, m_index, spell_m_id, spell_m_index) = match
            
            if at_spelldesc:
                spell_id = int(spell_desc_id)
                replacement = get_spell_description(spell_id, spells_df, spelleffect_df, spellmisc_df, spellduration_df, spellradius_df, spellname_df, spellauraoptions_df)
                description = description.replace(f"$@spelldesc{spell_id}", replacement, 1)
            elif at_spellname:
                spell_id = int(spellname_id)
                replacement = get_spell_name(spell_id, spellname_df)
                description = description.replace(f"$@spellname{spell_id}", replacement, 1)
            elif q_s:
                spell_id = int(q_s_id)
                ...
                index = int(s_index) - 1
                replacement = get_spelleffect_value(parent_spell_id, index, spelleffect_df)
                ...
                description = description.replace(f"${spell_id}s{spell_index}", replacement, 1)
            elif d_full:
                index = int(d_index) - 1 if d_index else 0
                replacement = get_spellduration_value(parent_spell_id, index, spellmisc_df, spellduration_df)
                ...
                replacement = get_spellduration_value(spell_id, index, spellmisc_df, spellduration_df)
                description = description.replace(f"${spell_d_id}d{spell_d_index}", replacement, 1)
            elif D_full:
                index = int(D_index) - 1 if D_index else 0
                replacement = get_spellduration_value(parent_spell_id, index, spellmisc_df, spellduration_df)
                description = description.replace(f"$D{D_index}", replacement, 1)
                ...
                replacement = get_effect_aura_period(parent_spell_id, index, spelleffect_df)
                ...
                description = description.replace(f"${spell_t_id}t{spell_t_index}", replacement, 1)
            elif T_full:
                index = int(T_index) - 1 if T_index else 0
                replacement = get_effect_aura_period(parent_spell_id, index, spelleffect_df)
                description = description.replace(f"$T{T_index}", replacement, 1)
                ...
                replacement = get_effect_radius(parent_spell_id, index, spelleffect_df, spellradius_df)
                ...
                replacement = get_effect_radius(spell_id, index, spelleffect_df, spellradius_df)
                description = description.replace(f"${spell_a_id}A{spell_a_index}", replacement, 1)
            elif a_lower_full:
                ...
                replacement = get_effect_radius(spell_id, index, spelleffect_df, spellradius_df)
                description = description.replace(f"${spell_a_lower_id}a{spell_a_lower_index}", replacement, 1)
            elif u_full:
                index = int(u_index) - 1 if u_index else 0
                replacement = get_cumulative_aura(parent_spell_id, index, spellauraoptions_df)
                ...
                description = description.replace(f"${spell_u_id}u{spell_u_index}", replacement, 1)
            elif m_full:
                index = int(m_index) - 1 if m_index else 0
                replacement = get_effect_base_points(parent_spell_id, index, spelleffect_df)
                ...
                description = description.replace(f"${spell_m_id}m{spell_m_index}", replacement, 1)
            else:
    ...
    for eval_match in eval_matches:
        try:
            if re.search('[a-zA-Z]', eval_match) is None:  # Only process if no letters are present
                evaluated_value = eval(eval_match)
                description = description.replace(f"${{{eval_match}}}", str(evaluated_value))
...
def get_spelleffect_value(spell_id, index, spelleffect_df):
    effect_row = spelleffect_df.loc[(spelleffect_df['SpellID'] == spell_id) & (spelleffect_df['EffectIndex'] == index)]
    if effect_row.empty:
...
def main():
    ...
    for i, spell_id in enumerate(spell_ids, 1):
        spell_id = int(spell_id.strip().replace(',', ''))  # Remove any extra commas or spaces
        try:
            description = get_spell_description(spell_id, spells_df, spelleffect_df, spellmisc_df, spellduration_df, spellradius_df, spellname_df, spellauraoptions_df)
            spell_name = get_spell_name(spell_id, spellname_df)
